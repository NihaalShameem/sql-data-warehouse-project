...
BEGIN 
	...
	BEGIN TRY
		...
		*,
		...
		prd_start_dt,
		DATEADD(DAY, -1, LEAD(prd_start_dt) OVER (PARTITION BY prd_key ORDER BY prd_start_dt)) AS prd_end_dt_test -- Calculate end date as one day before the next start date
		...
		SELECT 
		CASE WHEN cid LIKE 'NAS%' THEN  SUBSTRING(cid,4,LEN(cid)) -- removes 'NA's prefix if present
			ELSE cid
		END as cid,
